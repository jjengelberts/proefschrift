...
On the first row in both determinants of equation \ref{ch2.eq.s16g} there are orbital overlaps, like $s_{26}$, which do not occur in the overlap determinant $\left< \Psi_0 | \Psi_0 \right>$. Please note that these will be zero when orbital $\chi_6$ is chosen to be orthogonal to all other orbitals. 

If $\chi_6$ would be a virtual orbital, like in the case of excitation types $i \rightarrow a$ and $t \rightarrow a$, it can be chosen orthogonal (see the Appendix of reference \cite{koos1}). If that case, the overlap of $\Psi_0$ with $\Psi_{16}$ becomes:
\begin{equation}
\left < \Psi_0 | \Psi_{16} \right> =
\begin{array}{llllll}
 &  \chi_1 & \chi_2 & \chi_3 & \chi_4 & \chi_5 \\
...
For the other two excitation types, $i \rightarrow t$ and $t \rightarrow u$, orbital $\chi_6$ is not necessarily orthogonal to the other orbitals. In that case the first order cofactors contain overlaps that do not occur in $\left< \Psi_0 | \Psi_0 \right>$, and hence are not equal to density matrix elements. Therefore, Brillouin matrix elements of the type $i \rightarrow t$ and $t \rightarrow u$ will be constructed the usual way via L\"{o}wdins formula (equation \ref{ch2.eq.lowdindeterminants}).
...
\label{ch2.eq.p_is_s}
\end{equation}
In analogy with this, $P^{(u,t)}=S^{(u,a)}$ and $P^{(v,t)}=S^{(v,a)}$ by removing the fourth and fifth column, respectively, instead of the third. 
