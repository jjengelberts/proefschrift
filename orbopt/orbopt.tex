...
Since the orbitals in MO theory can be chosen orthogonal, this method attracted more attention. Its implementation in computer programs is more straightforward and computations on larger molecular systems are possible. Despite its computational hurdles, progress in Valence Bond theory continued \cite{vboverv1,vboverv2,vboverv3}. One of the reasons is the development of powerful computers. On the other hand several efficient methods to optimize Valence Bond wave functions, like Spin-Coupled Valence Bond (SCVB) \cite{scvb1,scvb2,scvb3} and Valence Bond Self-Consistent Field (VBSCF) \cite{vbscf1,vbscf2,koos1,zahid}, have been developed.

The most time consuming step in the optimization of the wave function is the optimization of the orbitals. Several methods to compute the gradient of the total energy with respect to orbital rotations have been developed. The most common one and widely applicable method is based the evaluation of Hamiltonian matrix elements between Slater determinants, as introduced by L\"{o}wdin \cite{lowdin}.

In another method, coined by Song \textit{et. al}, multiple Fock matrices are computed, one for each Slater determinant combination in the wave function \cite{song}. This approach is efficient for wave functions with a small number of Slater determinants. However, it is not as generally applicable as L\"{o}wdins method, because it can only be applied to wave functions without orthogonal orbitals \cite{xmvb}. Although in VB non-orthogonal orbitals are used explicitly, orthogonality can occur. For example, orbitals in the $\sigma$ system of benzene are, by molecular symmetry, orthogonal to the orbitals in the $\pi$ system.
...
\end{equation}
in which $\mathbf{H}$ is the Hamilton operator and $\Psi$ is the wave function. A wave function is said to be optimized once the lowest possible value $E$ has been found. For Valence Bond theory, such a wave function $\Psi$ is constructed from a linear combination of structures:
\begin{equation}
...
\label{ch2.eq.determ}
\end{equation}
where the spin-orbitals $\chi$ have a spatial part and a spin part which can be $\alpha$ or $\beta$. 
...
\label{ch2.eq.fock}
\end{equation}
in which $F_{ia}$ is the Fock matrix element of spin-orbitals $\chi_i$ and $\chi_a$, $h_{ia}$ is the one electron integral $\left< \chi_i | \mathbf{h}| \chi_a \right>$. The double sum on the right hand side is over two electron integrals (Coulomb and exchange) times elements of the density matrix $P^{(\sigma,\nu)}$. The density matrix is constructed from first order cofactors of the norm of the wave function, $\left< \Psi_0 | \Psi_0 \right>$. Each cofactor that contains the column labeled $\sigma$ and row labeled $\nu$ is added to the density matrix element $P^{(\sigma,\nu)}$. For a one determinant wave function, each density matrix element is equal to a single cofactor (\textit{vide infra}).

In multideterminant wave functions, three orbital classes can be distinguished. The first class contains those orbitals that occur in all determinants. These be denoted with labels $i$, $j$, $k$ and $l$. The second class contains orbitals that occur in some determinants, denoted with $t$, $u$, $v$ and $x$. The last class contains those orbitals that do not occur in the ground state wave function $\Psi_0$ and hence are virtual unoccupied orbitals, labeled $a$, $b$, $c$ and $d$. With those classes in mind, Roos \textit{et al.} published three expressions with Fock matrix elements for three types of excitations:
\begin{itemize}
...
The norm of this wave function $\left < \Psi_0 | \Psi_0 \right>$ is:
\begin{equation}
...
\label{ch2.eq.s0g}
\end{equation}
in which the $s$ elements are overlaps between orbitals. For example, $s_{34} = \left< \chi_3 | \chi_4 \right>$. For normalized wave functions, used in this chapter, the norm equals ``1''.
...
Brillouin matrix elements of the type $i \rightarrow t$ and $t \rightarrow t$ have first order cofactors like in equation \ref{ch2.eq.s016g}. Because these contain overlaps that do not occur in the norm $\left< \Psi_0 | \Psi_0 \right>$, they are not equal to density matrix elements. Therefore, Brillouin matrix elements of the type $i \rightarrow t$ and $t \rightarrow t$ will be constructed the usual way via L\"{o}wdins formula (equation \ref{ch2.eq.lowdindeterminants}).
...
As indicated before, Brillouin matrix elements of the type $i \rightarrow t$ and $t \rightarrow t$ are constructed with first order cofactors that contain overlaps that do not occur in the norm $\left< \Psi_0 | \Psi_0 \right>$. Hence, these elements are constructed with L\"{o}wdins formula (equation \ref{ch2.eq.lowdindeterminants}).
...
So far, the numerator of equation \ref{ch2.eq.anr} has been analyzed. In this section an approximation of the denominator, or the diagonal element in the Brillouin matrix, will be presented:
\begin{equation}
