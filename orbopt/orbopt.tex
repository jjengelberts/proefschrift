...
The essential idea in Valence Bond theory is that atoms share electrons to form bonds. This is achieved by spin-pairing singly occupied orbitals on different atoms, or molecular fragments. In other methods, like the Molecular Orbital theory \cite{hartree1,hartree2,hartree3,fock}, orbitals are not specifically bound to atoms or molecular fragments. In VB, the bond in H$_2$ can be described by the spin-coupling of the atomic $1s$ orbitals on both hydrogen atoms [H$\bullet$H$\bullet$]. The corresponding wave function consists of two determinants: $\Psi = |1s_{A}\overline{1s_{B}}| - |\overline{1s_{A}}1s_{B}|$. This determinant combination is called a structure. Ionic structures could be added to extend, or enhance the wave function: $|1s_{A}\overline{1s_{A}}|$ and $
|1s_{B}\overline{1s_{B}}|$ (see Chapter \chintro). Besides the use of multiple determinants in VB, non-orthogonal and localized orbitals are used explicitly. 

Since the orbitals in MO theory can be chosen orthogonal, this method attracted more attention. Its implementation in computer programs is more straight forward and computations on larger molecular systems are possible. Despite its computational hurdles, Valence Bond theory has kept a decent following through the years \cite{vboverv1,vboverv2,vboverv3}. One of the reasons is the development of powerful computers. On the other hand several efficient methods to optimize Valence Bond wave functions, like Spin-Coupled Valence Bond (SCVB) \cite{scvb1,scvb2,scvb3} and Valence Bond Self-Consistent Field (VBSCF) \cite{vbscf1,vbscf2,koos1,zahid}, have been developed.

Like VBSCF, Multi Configurational Self-Consistent Field (MCSCF) \cite{joop,mcscf,roos1,roos2}, is based on the Super CI method \cite{superci1,superci2} for the optimization of the wave function. In Super CI the wave function is expanded with singly excited structures, increasing the computational challenge even more. The size of the Hamiltonian matrix on structure basis will increase quadratically for the addition of each excitation. For MCSCF, Roos \textit{et al.} have shown that for certain single excitations it is possible to use elements of a single Fock matrix instead of computing the corresponding Hamiltonian matrix elements \cite{roos1}. The Fock matrix elements have the form:
\begin{equation}
...
\label{ch2.eq.fock}
\end{equation}
in which $F_{ia}$ is the Fock matrix element of spin-orbitals $\chi_i$ and $\chi_a$, $h_{ia}$ is the one electron integral $\left< \chi_i | \mathbf{h}| \chi_a \right>$. The double sum on the right hand side is over two electron integrals (Coulomb and exchange) times elements of the density matrix $P^{(\sigma,\nu)}$. The central question for this chapter is whether it is also possible to use a single Fock matrix for the optimization of the orbitals in VBSCF, although those are non-orthogonal. If multiple Hamiltonian matrix elements would be equal to elements from a single Fock matrix, it would not be necessary to construct these Hamiltonian matrix elements. This could save a considerable amount of computation time.
...
At first, the usability of Fock matrix elements instead of Hamiltonian matrix is analyzed by comparing the expression for Fock matrix elements, equation \ref{ch2.eq.fock}, with the expression for Hamiltonian matrix elements by L\"{o}wdin \cite{lowdin}. Following this analysis, the implementation of these Fock matrix elements in TURTLE \cite{turtle}, the VB module in GAMESS-UK \cite{gamess}, will be presented. After the implementation details, speed-up factors of some test calculations will be discussed and compared. The chapter is concluded with an outlook on future enhancements.  
...
\label{ch2.eq.eig}
\end{equation}
in which $\mathbf{H}$ and $\mathbf{S}$ are the Hamiltonian and overlap matrix in structure basis, respectively. Secondly, the orbitals are optimized, as will be explained below. After the orbital optimization the secular equation \ref{ch2.eq.eig} is solved again with the new orbitals until both sets of coefficients no longer change.
...
\end{equation}
where $C_p$ and $C_q$ are determinant coefficients, $\Delta_p$ and $\Delta_q$ are determinants, $S^{(i,k)}$ and $S^{(i,j,k,l)}$ are first and second order cofactors and $h_{ik}$ and the term between curly brackets are one and two electron integrals, respectively.
...
\subsubsection{Optimization of orbitals}
...
With the expansion into multiple determinants, three orbital classes can be distinguished. Firstly, orbitals that occur in all determinants, which will be denoted with labels $i$, $j$, $k$ and $l$. The second class contains orbitals that occur in some determinants, denoted with $t$, $u$, $v$ and $x$. The last class contains those orbitals that do not occur in the ground state wave function $\Psi_0$ and hence are virtual unoccupied orbitals, labeled $a$, $b$, $c$ and $d$.
...
\label{ch2.eq.hamilt}
\end{equation}
The Brillouin matrix has the same dimension as the $\mathbf{H}$ and $\mathbf{S}$ matrices in equation \ref{ch2.eq.geig}. Furthermore, this matrix is symmetric; in the TURTLE only the left lower triangle is stored in memory. For each extra excitation the $\mathbf{H}$, $\mathbf{S}$ and Brillouin matrices are extended with one extra row and column. Hence, the number of elements increases roughly quadratic with the number of excitations.
...
Van Lenthe \textit{et al.} presented a new method, referred to as approximated Newton-Raphson (Section \ref{ch2.sec.anr}), which requires less $\mathbf{H}$ and $\mathbf{S}$ matrix elements than regular Super CI \cite{koos1}, because elements of the type $\left < \Psi_{ia} | \mathbf{H} - E_0 | \Psi_{ib} \right >$ are not needed.
...
\end{itemize}
The fourth type of excitation, $t \rightarrow t$ from variably to variably occupied orbitals, is not needed in CASSCF, because all possible electron distributions in the variably occupied orbital space are already included in $\Psi_0$. In this section Brillouin matrix elements of the type $\left< \Psi_0 | \mathbf{H} | \Psi_{ia} \right>$ will be analyzed for all four types of excitations (L\"{o}wdins equation \ref{ch2.eq.lowdindeterminants}).
...
\end{equation}
The corresponding overlap $\left< \Psi_0 | \Psi_0 \right>$ is:
\begin{equation}
\begin{split}
\left< \Psi_0 | \Psi_0 \right>=&
\begin{array}{lllll}
...
The density matrix is constructed by looping through the determinants in equation \ref{ch2.eq.s02} and if the row and column labels of the density matrix element are present in the determinant, a cofactor is created and added to the density matrix element. Since orbitals $\chi_i$ and $\chi_j$ occur in all determinants, density matrix elements $P^{(i,i)}$ and $P^{(j,j)}$ get a contribution from cofactors of all these determinants. Other elements get a contribution from less determinants, a single one in this case. For example $P^{(t,x)}$, which gets a single cofactor from the second determinant in equation \ref{ch2.eq.s02}:
\begin{equation}
P^{(t,x)}=
\begin{array}{llll}
 &  \chi_i & \chi_j & \chi_u \\
...
\begin{equation}
\Psi_{ia} = |\chi_a\chi_j\chi_t\chi_u|+|\chi_a\chi_j\chi_v\chi_x|.
...
\begin{equation}
\left\{ [\chi_i\chi_a|\chi_t\chi_x] - [\chi_i\chi_x|\chi_t\chi_a] \right\} \cdot S^{(i,t,a,x)},
...
So, for the case with multiple determinants, the equality of equation \ref{ch2.eq.brilisfock} still holds. Only a single one electron integral contributes to the Brillouin matrix element multiplied with one or more first order cofactors. Since these cofactors are equal to the determinants of the overlap $\left< \Psi_0 | \Psi_0 \right>$, this factor will be ``1'' for a normalized wave function. For the two electron integral part, only Coulomb and exchange integrals in which $\chi_i$ and $\chi_a$ are present contribute. Furthermore, the second order cofactors are equal to density matrix elements, because the density matrix element will always have a single ``1'' on the diagonal of the extra row and column. This ``1'' arises from the fact that $\chi_i$ is orthogonal to all other orbitals.

\subsubsection{\label{ch2.sec.t-a}$t \rightarrow a$ from variably occupied to virtual orbitals}

The next type of excitation that will be investigated is from variably occupied to virtual orbitals ($t \rightarrow a$). For the analyses a two determinant wave function is defined from the variably occupied orbitals $\chi_t$, $\chi_u$, $\chi_v$, $\chi_x$, $\chi_y$ and $\chi_z$: 
\begin{equation}
\Psi_0=|\chi_t\chi_u\chi_v|+|\chi_x\chi_y\chi_z|.
\label{ch2.eq.wfexamp3}
\end{equation}
The corresponding overlap $\left< \Psi_0 | \Psi_0 \right>$ is:
\begin{equation}
\begin{split}
...
\begin{array}{llll}
 & \chi_t & \chi_u & \chi_v \\
 \chi_t & \multicolumn{1}{|l}{ 1 } & s_{ut} & \multicolumn{1}{l|}{ s_{vt} } \\
 \chi_u & \multicolumn{1}{|l}{ s_{tu} } & 1 & \multicolumn{1}{l|}{ s_{vu} } \\
 \chi_v & \multicolumn{1}{|l}{ s_{tv} } & s_{uv} & \multicolumn{1}{l|}{ 1 }
\end{array}+
\begin{array}{llll}
 & \chi_t & \chi_u & \chi_v \\
 \chi_x & \multicolumn{1}{|l}{ s_{tx} } & s_{ux} & \multicolumn{1}{l|}{ s_{vx} } \\
 \chi_y & \multicolumn{1}{|l}{ s_{ty} } & s_{uy} & \multicolumn{1}{l|}{ s_{vy} } \\
 \chi_z & \multicolumn{1}{|l}{ s_{tz} } & s_{uz} & \multicolumn{1}{l|}{ s_{vz} }
\end{array} \\
& + \begin{array}{llll}
 & \chi_x & \chi_y & \chi_x \\
 \chi_t & \multicolumn{1}{|l}{ s_{xt} } & s_{yt} & \multicolumn{1}{l|}{ s_{zt} } \\
 \chi_u & \multicolumn{1}{|l}{ s_{xu} } & s_{yu} & \multicolumn{1}{l|}{ s_{zu} } \\
 \chi_v & \multicolumn{1}{|l}{ s_{xv} } & s_{yv} & \multicolumn{1}{l|}{ s_{zv} }
\end{array}+
\begin{array}{llll}
 & \chi_x & \chi_y & \chi_z \\
 \chi_x & \multicolumn{1}{|l}{ 1 } & s_{yx} & \multicolumn{1}{l|}{ s_{zx} } \\
 \chi_y & \multicolumn{1}{|l}{ s_{xy} } & 1 & \multicolumn{1}{l|}{ s_{zy} } \\
 \chi_z & \multicolumn{1}{|l}{ s_{xz} } & s_{yz} & \multicolumn{1}{l|}{ 1 }
\end{array}.
\end{split}
\label{ch2.eq.s03}
\end{equation}
The singly excited state consists of one determinant, the one in $\Psi_0$ that contains $\chi_t$, which is replaced by $\chi_a$. Since $\chi_t$ does not occur in the second determinant of $\Psi_0$, it is not present in $\Psi_{ta}$:
\begin{equation}
\Psi_{ta}=|\chi_a\chi_u\chi_v|.
\end{equation}
The overlap of $\Psi_0$ with this $\Psi_{ta}$ is:
\begin{equation}
\left< \Psi_0 | \Psi_{ta} \right> =
\begin{array}{llll}
 & \chi_t & \chi_u & \chi_v \\
 \chi_a & \multicolumn{1}{|l}{ 0 } & 0 & \multicolumn{1}{l|}{ 0 } \\
 \chi_u & \multicolumn{1}{|l}{ s_{tu} } & 1 & \multicolumn{1}{l|}{ s_{vu} } \\
 \chi_v & \multicolumn{1}{|l}{ s_{tv} } & s_{uv} & \multicolumn{1}{l|}{ 1 }
\end{array}+
\begin{array}{llll}
...
After analysis of the overlap determinants in equation \ref{ch2.eq.psi0_ta}, it is clear that two one electron integrals will add to the Brillouin matrix element, $h_{ta}\cdot S^{(t,a)}$ and $h_{xa}\cdot S^{(x,a)}$. Two sets of two electron integrals contribute to the Brillouin matrix element:
\begin{equation}
\sum_{\sigma\nu} \left\{ [\chi_t\chi_a | \chi_\sigma\chi_\nu] - [\chi_t\chi_\nu | \chi_\sigma\chi_a ] \right\} \cdot S^{(t,\sigma,a,\nu)},
\end{equation}
and
\begin{equation}
\sum_{\sigma\nu} \left\{ [\chi_x\chi_a | \chi_\sigma\chi_\nu] - [\chi_x\chi_\nu | \chi_\sigma\chi_a ] \right\} \cdot S^{(x,\sigma,a,\nu)}.
\end{equation}
The structure of the density matrix elements, created from the determinants in equation \ref{ch2.eq.s03}, is different from the ones in the $i \rightarrow a$ case. The determinants here do not have a single ``1'' on the diagonal and are therefore not equal to the second order cofactors. This means that the second order cofactors need to be calculated and that the Brillouin matrix element is created in the regular way.

\subsubsection{\label{ch2.sec.i-t}$i \rightarrow t$ from (always) doubly to variably occupied orbitals}

For the examination of the case where an excitation takes place from the doubly occupied orbitals the wave function of equation \ref{ch2.eq.wfexamp2} and the corresponding overlap in equation \ref{ch2.eq.s02} will be used.

The excitation that will be examined is from $\chi_i$ to $\chi_t$. Since $\chi_t$ is already occupied in the first determinant of $\Psi_0$, only the second determinant, with $\chi_i$ replaced by $\chi_t$, will be present in $\Psi_{it}$:
\begin{equation}
\Psi_{it} = |\chi_t\chi_j\chi_v\chi_x|.
\end{equation}
The overlap of $\Psi_0$ with $\Psi_{it}$ is:
\begin{equation}
\left<\Psi_0|\Psi_{it} \right> =
\begin{array}{lllll}
 &  \chi_i & \chi_j & \chi_t & \chi_u \\
 \chi_t & \multicolumn{1}{|l}{ 0 } & 0 & 1 & \multicolumn{1}{l|}{ s_{ut} } \\
 \chi_j & \multicolumn{1}{|l}{ 0 } & 1 & 0 & \multicolumn{1}{l|}{ 0 } \\
...
\begin{array}{lllll}
 ...
 \chi_x & \multicolumn{1}{|l}{ 0 } & 0 & s_{vx} & \multicolumn{1}{l|}{ 1 }
\end{array}.
\label{ch2.eq.psi0_it}
\end{equation}

Multiple one electron integrals will add to the Brillouin matrix element, since a lot of first order cofactors can be created from the determinants in equation \ref{ch2.eq.psi0_it}. One of these one electron integrals is $h_{iv}$. This one electron integral is multiplied by two first order cofactors:
\begin{equation}
h_{iv} \cdot
\left( \begin{array}{llll}
 & \chi_j & \chi_t & \chi_u \\
 \chi_t & \multicolumn{1}{|l}{ 0 } & 1 & \multicolumn{1}{l|}{ s_{ut} } \\
 \chi_j & \multicolumn{1}{|l}{ 1 } & 0 & \multicolumn{1}{l|}{ 0 } \\
 \chi_x & \multicolumn{1}{|l}{ 0 } & s_{tx} & \multicolumn{1}{l|}{s_{ux}}
\end{array}+
\begin{array}{llll}
 & \chi_j & \chi_v & \chi_x \\
 \chi_t & \multicolumn{1}{|l}{ 0 } & s_{vt} & \multicolumn{1}{l|}{ s_{xt} } \\
 \chi_j & \multicolumn{1}{|l}{ 1 } & 0 & \multicolumn{1}{l|}{ 0 } \\
 \chi_x & \multicolumn{1}{|l}{ 0 } & s_{vx} & \multicolumn{1}{l|}{ 1 }
\end{array} \right).
\end{equation}
Both cofactors are no subdeterminants of the determinants in equation \ref{ch2.eq.s02}. This means that they need to be created. In other words, a Brillouin matrix element is created the regular way for this type of excitation.

\subsubsection{\label{ch2.sec.t-t}$t \rightarrow t$ from variably to variably occupied orbitals}

For the last type of excitation, the wave function and corresponding overlap in equations \ref{ch2.eq.wfexamp3} and \ref{ch2.eq.s03}, respectively, are used. From this wave function orbital $\chi_t$ is excited to orbital $\chi_y$. Since $\chi_y$ is already present in the second Slater determinant of $\Psi_0$, only the first determinant, with $\chi_t$ replaced by $\chi_y$, is present in the singly excited state wave function $\Psi_{ty}$:
\begin{equation}
\Psi_{ty} = |\chi_y\chi_u\chi_v|.
\end{equation}
The overlap matrix of $\Psi_0$ with $\Psi_{ty}$ is:
\begin{equation}
\left< \Psi_0 | \Psi_{ty} \right> =
\begin{array}{llll}
 & \chi_t & \chi_u & \chi_v \\
 \chi_y & \multicolumn{1}{|l}{ s_{ty} } & s_{uy} & \multicolumn{1}{l|}{ s_{vy} } \\
 \chi_u & \multicolumn{1}{|l}{ s_{tu} } & 1 & \multicolumn{1}{l|}{ s_{vu} } \\
...
\begin{array}{llll}
 ...
 \chi_v & \multicolumn{1}{|l}{ s_{xv} } & s_{yv} & \multicolumn{1}{l|}{ s_{zv} }
\end{array}.
\label{ch2.eq.psi0_tt}
\end{equation}

One of the one electron integrals that contributes to the Brillouin matrix element is $h_{xu} \cdot S^{(x,u)}$:
\begin{equation}
h_{xu} \cdot S^{(x,u)} = h_{xu} \cdot 
\begin{array}{lll}
 & \chi_y & \chi_z \\
 \chi_y & \multicolumn{1}{|l}{ 1 } & \multicolumn{1}{l|}{ s_{zy} } \\
 \chi_v & \multicolumn{1}{|l}{ s_{yv} } & \multicolumn{1}{l|}{ s_{zv} }
\end{array}.
\end{equation} 
Like in the previous case, there is no element in the density matrix that is equal to this cofactor. Therefore, also in this last case a normal Brillouin matrix element needs to be constructed.

\subsubsection{\label{ch2.sec.delocal}Localized VB wave functions}
In TURTLE it is possible to restrict orbitals to certain atoms, or fragments of the molecule. This type of wave function is called localized. Within each fragment, the doubly occupied and virtual orbitals are orthogonal to all other orbitals on the same fragment. However, between fragments, overlap of orbitals can occur. This means that doubly occupied orbitals from one fragment are no longer necessarily orthogonal to the orbitals in another fragment. In that case the needed single ``1'' on the diagonal of the determinant from which the density matrix element is constructed will not be present. Due to non-orthogonality non-zero elements will show up on the row and column. 
...
Up till here, the numerator of equation \ref{ch2.eq.anr} has been analyzed. In this section an approximation of the denominator, or the diagonal element in the Brillouin matrix, will be presented:
\begin{equation}
