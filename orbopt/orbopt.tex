...
At first, the usability of Fock matrix elements for the construction of the Hamiltonian matrix is analyzed by comparing the expression for Fock matrix elements, equation \ref{ch2.eq.fock}, with the expression for Hamiltonian matrix elements by L\"{o}wdin \cite{lowdin}:
\begin{equation}
...
For the examination of the case where an excitation takes place from the doubly occupied orbitals, a two determinant wave function is constructed containing $\chi_i \equiv \psi_i$, $\chi_j \equiv \overline{\psi_i}$, $\chi_t$, $\chi_u$, $\chi_v$ and $\chi_x$:
\begin{equation}
\Psi_0 = |\chi_i\chi_j\chi_t\chi_u| + |\chi_i\chi_j\chi_v\chi_x|.
\end{equation}
The overlap for this wave function has already been used before (equation \ref{ch2.eq.s02}).
...
In TURTLE it is possible to restrict orbitals to certain atoms, or fragments of the molecule. This type of wave function is called localized. Within each fragment, the doubly occupied and virtual orbitals are orthogonal to each other. However, between set, overlap of orbitals can occur. This means that doubly occupied orbitals from one fragment are no longer necessarily orthogonal to the orbitals in another fragment. In that case the needed single ``1'' on the diagonal of the determinant from which the density matrix element is constructed will not be present. Due to non-orthogonality non-zero elements will show up on the row and column. 

Sometimes, however, orbitals from different fragments can still be orthogonal due to the symmetry in the molecule (see the example in Section \ref{ch2.sec.aromat}). In that case the elements of the Fock matrix can be used instead of calculating the Brillouin matrix elements.

\subsubsection{\label{ch2.sec.denominator}Approximation Of The Denominator}

Up till here, the numerator of equation \ref{ch2.eq.anr} has been analyzed. In this section an approximation of the denominator, or the the diagonal element in the Brillouin matrix, will be presented:
\begin{equation}
