...
In the following years several methods were developed that overcame an essential shortcoming of the single determinant Hartree-Fock wave function, \textit{i.e.} the lack of electron correlation. With the expression of the wave function in just a single determinant, the electron-electron interaction is treated in an average way. For a more accurate expression, the instantaneous interaction between the electrons must be taken into account. Electronic structure methods that take care of electron-electron interaction are called electron correlation methods. Two of these methods, the Configuration Interaction Method (CI) \cite{shavitt1,shavitt2} and the Multi-Configurational Self-Consistent Field Method (MCSCF) \cite{daswahl,wahldasbook,mcscf,roos1,roos2}, will be introduced in Section \ref{ch1.sec.wavefunctions}, together with their VB analogues. 
