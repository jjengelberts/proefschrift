...
Molecules react differently to an external magnetic field. In benzene, for example, the center of the ring is shielded, while the edges of the molecule are deshielded.  Voor cyclisch geconjugeerde systemen - afhankelijk van het aantal pi electronen - dia, of paratrope ringcurrent (positieve en negatieve NICS waarden).
...
The total ring current is independent of the choice of the gauge, \textit{i.e.} the formal center of rotation. However, this is not true for individual orbital contributions \cite{ipso1}. In the ipsocentric model of Continuous Transformation of Origin of Current-Density Diamagnetic Zero (CTOCD-DZ) the induced current density is calculated at each point with the gauge at that point \cite{ipso1,ipso2}. With this method the induced ring current per orbital can be calculated.

In this ipsocentric formulation, the current densities may be analyzed in terms of the accessibility of excited states via translational and rotational transitions. Uitleg en opnemen van formule (16) uit ipso1. linear momentum operator doet diatrope. angular momentum doet paratrope.




For two example molecules, benzene (aromatic) and planar cyclooctatetraene (anti-aromatic), the orbital scheme with these transitions is shown for the $\pi$ system in Figure \ref{ch1.fig.orbital_scheme}. 
\begin{figure}[hb]
\center
