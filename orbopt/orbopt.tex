...
Secondly, as indicated in the introduction, the Fock matrix approach of Song \textit{et al.} has the drawback that it can not be used for wave functions with orthogonal orbitals \cite{song,xmvb}. With the Fock matrix approach presented in this chapter, only orthogonal orbitals can be optimized with a cheap Fock matrix. So, combining the two methods seems to be a logical choice. The optimization algorithm in TURTLE could be modified in such a way that it consists of two separate steps for the orbital optimization, one for the orthogonal orbitals (with the Fock method presented here) and one for the non-orthogonal orbitals (with the Fock method as introduced by Song). Since orthogonal orbitals prevent the use of Songs Fock method, they could be frozen during the optimization step for the non-orthogonal orbitals.
