...
\label{ch2.eq.fock}
\end{equation}
in which $F_{ia}$ is the Fock matrix element, $\chi_i$, $\chi_a$, $\chi_\sigma$ and $\chi_\nu$ are spin-orbitals, $h_{ia}$ is the one electron integral $\left< \chi_i | \mathbf{h}| \chi_a \right>$. The double sum on the right hand side is over two electron integrals (Coulomb and exchange) times elements of the density matrix $P^{(\sigma,\nu)}$. 

Compared to Hamiltonian matrix elements, the construction of a Fock matrix saves a substantial amount of computation time. Firstly, only a single Fock matrix is used to construct multiple Hamiltonian matrix elements. Secondly, the Fock matrix is based on the density matrix. This density matrix is constructed by looping through each determinant combination in the wave function. The Hamiltonian matrix element is computed for the Super CI \cite{superci1,superci2} wave function, an expansion of the wave function with all singly excited structures and hence (much) more determinants. The central question for this chapter is whether it is also possible to use a single Fock matrix for the optimization of the orbitals in VBSCF, although those are non-orthogonal.
...
At first, the usability of Fock matrix elements instead of Hamiltonian matrix is analyzed by comparing the expression for Fock matrix elements, equation \ref{ch2.eq.fock}, with the expression for Hamiltonian matrix elements by L\"{o}wdin \cite{lowdin}:
\begin{equation}
\left< \Delta_p | \hat{H} | \Delta_q \right> = \sum_{ik} h_{ik} \cdot S^{(i,k)} + \sum_{i<j,k<l} \left\{ \left[ \chi_i \chi_k | \chi_j \chi_l \right] - \left[ \chi_i \chi_l | \chi_j \chi_k \right] \right\} \cdot S^{(i,j,k,l)},
...
\begin{array}{llll}
 & \chi_x & \chi_y & \chi_z \\
 \chi_a & \multicolumn{1}{|l}{ 0 } & 0 & \multicolumn{1}{l|}{ 0 } \\
 \chi_u & \multicolumn{1}{|l}{ 0 } & s_{yu} & \multicolumn{1}{l|}{ s_{zu} } \\
 \chi_v & \multicolumn{1}{|l}{ 0 } & s_{yv} & \multicolumn{1}{l|}{ s_{zv} }
\end{array}.
