...
In this ipsocentric formulation, the current densities may be analyzed in terms of the accessibility of excited states via translational and rotational transitions. In Figure \ref{ch1.fig.orbital_scheme} it can be seen that the diamagnetic ring current in benzene (a) is caused by translational transitions (from the linear momentum operator \cite{ipso1}), while the paramagnetic ring current in planar cyclooctatetraene (b) is caused by rotational transitions (from the angular momentum operator \cite{ipso1}).
\begin{figure}[hb]
\center
